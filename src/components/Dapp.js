...
class DApp extends Component {
  ...
  async information(){
    ...
    if(ownerAddress != "" &&  price != ""){
      ...
      console.log(this.state.listState,this.state.price, this.state.listIndex)
    }
  }
    ...
    return (
      <div>
        ...
        <InputGroup.Text id="basic-addon1">Axie ID</InputGroup.Text>
          ...
          <Button variant={this.state.enablebutton ? "success" : "danger"} id="button-addon2"  onClick={this.state.enablebutton ? ()=>this.settleAuction(): ()=>this.alertmessage()}  >{this.state.enablebutton ? "buy" : "inut id"} </Button>
        </InputGroup><br/>
        <h3>
          {this.state.message}
        </h3><br/><br/>
